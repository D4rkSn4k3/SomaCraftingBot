...
Global $WHDepositButtonX = $DialogX + 287
Global $WHDepositButtonY = $DialogY + 97
;>>>>>> Crafting
$Map2 = 0
Send("{LCTRL}")
				...
				For $i = 1 To UBound($InventoryBag) - 1 Step 1 ; loop through slots of  invent bag
					If $InventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $InventoryBag[$i][0] <> 0 Then
						...
						If $InventoryBag[$i][2] > 1 Then
							...
							ExitLoop
							Else
							MsgBox(0, "", "Not enough Space in WH" & $SA[$d][0])
					...
					If $i = UBound($InventoryBag) - 1 And $InventoryBag[$i][0] <> $SA[$CurrentScriptRow][$d] Then
						$boton = 0
						MsgBox(0, "", "Could Not Find Item " & $SA[$d][0])
						$RowFinished = True
					EndIf
				Next
			Next
