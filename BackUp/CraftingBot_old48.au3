...
Global $ScriptViewListArray[1][1]
#EndRegion ################## Arrays #######################################################################
...
Func Gui()
	...
	;Save Load
	$CurrentScript = GUICtrlCreateLabel("Script: ", 16, 16, 370, 17)
	...
	;ScriptViewList
	$ScriptViewList = GUICtrlCreateListView("Oneeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee", 8, 264, 425, 136, BitOR($GUI_SS_DEFAULT_LISTVIEW, $LVS_NOCOLUMNHEADER))
...
Func LoadScript()
	...
	Else
		...
		$ScriptViewListArray[0][0] = $IniSectionNames[0]
		For $d = 1 To $IniSectionNames[0] Step 1
			If $IniSectionNames[0] > UBound($ScriptViewListArray) Then ; Increase size of array to fit all sections
				ReDim $ScriptViewListArray[$IniSectionNames[0] + 1][UBound($ScriptViewListArray, 2)]
			EndIf
			$Section = IniReadSection($sFileOpenDialog, $d)
			For $i = 0 To $Section[0][0] - 1 Step 1
				If $Section[0][0] > UBound($ScriptViewListArray, 2) Then
					ReDim $ScriptViewListArray[UBound($ScriptViewListArray)][$Section[0][0] + 1]
				EndIf
				$ScriptViewListArray[$d][$i] = IniRead($sFileOpenDialog, $d, $i, 0)
			Next
		Next
		GuiUpdate()
		MobSelection()
...
Func SaveScript()
	...
	Else
		...
		For $i = 1 To $ScriptViewListArray[0][0] Step 1 ; Loop through the ScriptArray

			If StringInStr($ScriptViewListArray[$i][0], "Navigate") > 1 Then

				For $n = 0 To UBound($ScriptViewListArray, 2) - 1 Step 1
					If $ScriptViewListArray[$i][$n] <> "" Then
						IniWrite($sFileSaveDialog, $i, $n, $ScriptViewListArray[$i][$n])
					EndIf
;~ 					msgbox(0,"",$n & "   " & UBound($ScriptViewListArray,2))
				Next
...
Func AddNavNodesToScript()
	...
	If ($NavNodes[0][0] * 2) + 2 > UBound($ScriptViewListArray, 2) Then
		ReDim $ScriptViewListArray[UBound($ScriptViewListArray) + 1][($NavNodes[0][0] * 2) + 2]
	Else
		ReDim $ScriptViewListArray[UBound($ScriptViewListArray) + 1][UBound($ScriptViewListArray, 2)]
	EndIf
	For $i = 1 To $NavNodes[0][0] Step 1 ; Add X Coords to ScriptView
		$ScriptViewListArray[$ScriptViewListArray[0][0]][$i + 1] = $NavNodes[$i][0]
	Next
	$ScriptViewListArray[$ScriptViewListArray[0][0]][1] = $NavNodes[0][0] ; Save a marker for the beginning of the Y Coords
	...
	Next
	$ScriptViewListArray[$ScriptViewListArray[0][0]][0] = $ScriptViewListArray[0][0] & " | " & "Navigate " & $NavNodes[0][0] & " Nodes"
	GuiUpdate()
EndFunc   ;==>AddNavNodesToScript

Func DeleteLast()
	If $ScriptViewListArray[0][0] > 0 Then
		ReDim $ScriptViewListArray[$ScriptViewListArray[0][0]][UBound($ScriptViewListArray, 2)]
		$ScriptViewListArray[0][0] -= 1
		GuiUpdate()
	EndIf
EndFunc   ;==>DeleteLast
...
Func GuiUpdate()
;~ 		GUISetState(@SW_LOCK, $CraftingBotGUI)
	_GUICtrlListView_DeleteAllItems(GUICtrlGetHandle($ScriptViewList))
	_GUICtrlListView_AddArray($ScriptViewList, $ScriptViewListArray)
	_GUICtrlListView_DeleteItem($ScriptViewList, 0)
;~ 		GUISetState(@SW_UNLOCK, $CraftingBotGUI)
EndFunc   ;==>GuiUpdate

Func Test()
;~ 	$SearchString = StringInStr($ScriptViewListArray[1][0], "Navigate")
	If StringInStr($ScriptViewListArray[1][0], "Navigate") > 1 Then
		MsgBox(0, "", "   ff")
...
$CurrentScriptRow += 1
		;-------------------------------------------------------------------------
		If StringInStr($ScriptViewListArray[$CurrentScriptRow][0], "Navigate") > 1 Then
				For $i = 1 To $MaxNodes Step 1
		$NavNodes[0][0] = 0
		$NavNodes[$i][0] = 0
		$NavNodes[$i][1] = 0
	Next
			$NavNodes[$n][0] = $ScriptViewListArray[$CurrentScriptRow][1] ; Save Number of Nodes
			For $n = 1 To $ScriptViewListArray[$CurrentScriptRow][1] Step 1
				If $ScriptViewListArray[$CurrentScriptRow][$n] <> "" Then
					$NavNodes[$n][0] = $ScriptViewListArray[$CurrentScriptRow][$n+1]
					$NavNodes[$n][1] = $ScriptViewListArray[$CurrentScriptRow][$n+1+$ScriptViewListArray[$i][1]]
				EndIf
			Next
		EndIf
		;-------------------------------------------------------------------------
