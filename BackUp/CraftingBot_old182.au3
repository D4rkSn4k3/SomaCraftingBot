...
Func Gui()
	...
	GUICtrlSetData($ItemsSelectAction, "Craft: Potion Table|WH: Withdraw Amount|WH: Deposit Amount")
...
Func ItemAddButton()
	...
	ElseIf GUICtrlRead($ItemsSelectAction) = "WH: Deposit Amount" Then
...
Func AddWHDepositAmountToScript()
	...
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "WH: Deposit Amount"
	$SA[$SA[0][0]][1] = GUICtrlRead($ItemID1)
	$SA[$SA[0][0]][2] = GUICtrlRead($ItemID2)
	$SA[$SA[0][0]][3] = GUICtrlRead($ItemID3)
	$SA[$SA[0][0]][4] = GUICtrlRead($ItemID4)
...
Func ScriptLoop()
	...
	If $ScriptFinished = False And $SA[0][0] > 0 Then
		;=============================WH Deposit Amount============================================
		If StringInStr($SA[$CurrentScriptRow][0], "WH: Deposit Amount") > 1 Then
			$LIndex = GUICtrlCreateListViewItem(">>> " & $CurrentScriptRow & " WH: Deposit Amount", $ScriptConsole)
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			$InventSpacesFilledPre = ""
				...
				For $i = 1 To UBound($InventoryBag) - 1 Step 1 ; loop through slots of  invent bag
					...
					If $InventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $InventoryBag[$i][0] <> 0 Then
						If Not WinActive($NameOfSomaWindow) Then
							...
							WinWaitActive($NameOfSomaWindow)
						EndIf
						_KDMemory_WriteProcessMemory($handles, 0x053598C, 'DWORD', $i - 1, $offsets = 0)
...
;~ 								$boton = 0
								MsgBox(0, "", "Not enough Space in WH1" & $SA[$CurrentScriptRow][$d])
								$RowFinished = True
								ExitLoop 2
							EndIf
						Else
