...
HotKeySet("!f", "ScanShopInventoryArrayDisplay")
HotKeySet("!z", "Test")
#EndRegion ################## HotKeys #######################################################################
...
Global $WHDepositButtonX = $DialogX + 215
Global $WHDepositButtonY = $DialogY + 187
Global $ShopSellButtonX = $DialogX + 215
Global $ShopSellButtonY = $DialogY + 207
Global $ShopBuyButtonX = $DialogX + 215
Global $ShopBuyButtonY = $DialogY + 307
;>>>>>> Crafting
$Map2 = 0
Send("{LCTRL}")
...
Func ScanShopInventoryArrayDisplay()
	ScanShopInventory()
	_ArrayDisplay($ShopInventory, "Shop Inventory")
EndFunc   ;==>ScanShopInventoryArrayDisplay
...
Func ScanShopInventory()
	Global $ScanShopInventoryIDOffset[2] = [0x0, 0x10]
	Global $ScanShopInventoryNameOffset[3] = [0x0, 0x30, 0x0]
	Global $ScanShopInventoryWeightOffset[2] = [0x0, 0x1a]
	Global $ScanShopInventoryDuraAmountOffset[2] = [0x0, 0x6]
	For $i = 1 To UBound($ShopInventory) - 1 Step 1
		If $i > 1 Then
			$ScanShopInventoryIDOffset[0] += 4
			$ScanShopInventoryNameOffset[0] += 4
			$ScanShopInventoryWeightOffset[0] += 4
			$ScanShopInventoryDuraAmountOffset[0] += 4
		EndIf
		$ShopInventory[$i][0] = _KDMemory_ReadProcessMemory($handles, 0x0536258, 'WORD', $ScanShopInventoryIDOffset)
		$ShopInventory[$i][1] = _KDMemory_ReadProcessMemory($handles, 0x0536258, 'char[30]', $ScanShopInventoryNameOffset)
		$ShopInventory[$i][2] = _KDMemory_ReadProcessMemory($handles, 0x0536258, 'WORD', $ScanShopInventoryWeightOffset)
		$ShopInventory[$i][3] = _KDMemory_ReadProcessMemory($handles, 0x0536258, 'WORD', $ScanShopInventoryDuraAmountOffset)
	Next
EndFunc   ;==>ScanShopInventory
...
Func Gui()
	...
	GUICtrlSetData($ItemsSelectAction, "Craft: Loom|Craft: Potion Table|WH: Withdraw Amount|WH: Deposit All|Shop: Sell All|Shop: Buy Amount")
	...
	$AddLoop = GUICtrlCreateButton("Add [Loop]", 93, 440, 75, 25)
	GUICtrlSetOnEvent($AddKey, "OpenSendKeyGUI")
	GUICtrlSetOnEvent($AddLoop, "AddLoopToScript")
...
Func ItemAddButton()
	...
	ElseIf GUICtrlRead($ItemsSelectAction) = "WH: Deposit All" Then
		AddWHDepositAmountToScript()
	ElseIf GUICtrlRead($ItemsSelectAction) = "Shop: Sell All" Then
		AddShopSellAllToScript()
	ElseIf GUICtrlRead($ItemsSelectAction) = "Shop: Buy Amount" Then
		AddShopBuyAmountToScript()
			ElseIf GUICtrlRead($ItemsSelectAction) = "Craft: Loom" Then
		AddShopBuyAmountToScript()
	EndIf
...
Func AddLoopToScript()
	...
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "Loop"
	GuiUpdate()
	$CurrentScriptRow = 0
	$ScriptFinished = True
	ScriptLoop()
EndFunc   ;==>AddLoopToScript

Func AddWHDepositAmountToScript()
	$SA[0][0] += 1 ; Count of rows in ScriptArray
	If 9 > UBound($SA, 2) Then ; If data is too big for current size of SA Columns
		ReDim $SA[UBound($SA) + 1][9] ; increases size of rows by one for new data and columns for data size
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "WH: Deposit All"
	$SA[$SA[0][0]][1] = GUICtrlRead($ItemID1)
	$SA[$SA[0][0]][2] = GUICtrlRead($ItemID2)
	$SA[$SA[0][0]][3] = GUICtrlRead($ItemID3)
	$SA[$SA[0][0]][4] = GUICtrlRead($ItemID4)
	...
	$ScriptFinished = True
	ScriptLoop()
EndFunc   ;==>AddWHDepositAmountToScript

Func AddShopSellAllToScript()
	$SA[0][0] += 1 ; Count of rows in ScriptArray
	If 9 > UBound($SA, 2) Then ; If data is too big for current size of SA Columns
		ReDim $SA[UBound($SA) + 1][9] ; increases size of rows by one for new data and columns for data size
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "Shop: Sell All"
	$SA[$SA[0][0]][1] = GUICtrlRead($ItemID1)
	$SA[$SA[0][0]][2] = GUICtrlRead($ItemID2)
	$SA[$SA[0][0]][3] = GUICtrlRead($ItemID3)
	$SA[$SA[0][0]][4] = GUICtrlRead($ItemID4)
	...
	$ScriptFinished = True
	ScriptLoop()
EndFunc   ;==>AddShopSellAllToScript

Func AddShopBuyAmountToScript()
	$SA[0][0] += 1 ; Count of rows in ScriptArray
	If 9 > UBound($SA, 2) Then ; If data is too big for current size of SA Columns
		ReDim $SA[UBound($SA) + 1][9] ; increases size of rows by one for new data and columns for data size
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "Shop: Buy Amount"
	$SA[$SA[0][0]][1] = GUICtrlRead($ItemID1)
	$SA[$SA[0][0]][2] = GUICtrlRead($ItemID2)
	$SA[$SA[0][0]][3] = GUICtrlRead($ItemID3)
	$SA[$SA[0][0]][4] = GUICtrlRead($ItemID4)
	...
	$ScriptFinished = True
	ScriptLoop()
EndFunc   ;==>AddShopBuyAmountToScript

Func AddLoomCraftingToScript()
	$SA[0][0] += 1 ; Count of rows in ScriptArray
	If 9 > UBound($SA, 2) Then ; If data is too big for current size of SA Columns
		ReDim $SA[UBound($SA) + 1][9] ; increases size of rows by one for new data and columns for data size
	Else
...
Sleep($DialogSleep)
	If $ScriptFinished = False And $SA[0][0] > 0 Then
		;=============================Shop: Buy Amount============================================
		If StringInStr($SA[$CurrentScriptRow][0], "Shop: Buy Amount") > 1 Then
			$LIndex = GUICtrlCreateListViewItem(">>> " & $CurrentScriptRow & " Shop: Buy Amount", $ScriptConsole)
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
				For $i = 1 To UBound($ShopInventory) - 1 Step 1 ; loop thourgh slots of wh invent bag
					ScanShopInventory()
					If $ShopInventory[$i][0] = $SA[$CurrentScriptRow][$d] And $ShopInventory[$i][0] <> 0 Then
						If Not WinActive($NameOfSomaWindow) Then
							WinActivate($NameOfSomaWindow)
							WinWaitActive($NameOfSomaWindow)
						EndIf
						_KDMemory_WriteProcessMemory($handles, 0x0536270, 'DWORD', $i - 1, $offsets = 0)
						Sleep($DialogSleep)
						If ($ShopInventory[$i][2] * $SA[$CurrentScriptRow][$d + 4]) + $BWCurrent <= $BWMax Then ; Weight * Amount = < available BW
							MouseClick("Left", $ShopBuyButtonX, $ShopBuyButtonY, 5)
							Sleep($DialogSleep)
							$AmountDialog = _KDMemory_ReadProcessMemory($handles, 0x0534940, 'DWORD', $offsets = 0)
							If $AmountDialog = 1 Then
						...
						ExitLoop
					EndIf
					If $i = UBound($ShopInventory) - 1 And $d = 4 And $ShopInventory[$i][0] <> $SA[$CurrentScriptRow][$d] Then
						$boton = 0
						MsgBox(0, "", "Could Not Find Item " & $SA[$CurrentScriptRow][$d])
						$RowFinished = True
					EndIf
				Next
			Next
			$RowFinished = True
			;=============================Loop============================================
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "Loop") > 1 Then
			$LIndex = GUICtrlCreateListViewItem(">>> " & $CurrentScriptRow & " Loop", $ScriptConsole)
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			$CurrentScriptRow = 0
			$RowFinished = True
			;=============================Shop: Sell All============================================
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "Shop: Sell All") > 1 Then
			$LIndex = GUICtrlCreateListViewItem(">>> " & $CurrentScriptRow & " Shop: Sell All", $ScriptConsole)
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
				$PlayerInvWHSlot = 1
				For $i = 1 To UBound($InventoryBag) - 1 Step 1 ; loop thourgh slots of wh invent bag
					ScanInventoryBag()
					If $InventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $InventoryBag[$i][0] <> 0 Then
						If Not WinActive($NameOfSomaWindow) Then
							WinActivate($NameOfSomaWindow)
							WinWaitActive($NameOfSomaWindow)
						EndIf
;~ 						msgbox(0,"",$i & @crlf & $PlayerInvWHSlot & @crlf & $i - $PlayerInvWHSlot)
;~ 						msgbox(0,"","fired")
						_KDMemory_WriteProcessMemory($handles, 0x053626C, 'DWORD', $i - $PlayerInvWHSlot, $offsets = 0) ; Select the item
						Sleep($DialogSleep)
						MouseClick("Left", $ShopSellButtonX, $ShopSellButtonY, 5)
						Sleep($DialogSleep)
						$AmountDialog = _KDMemory_ReadProcessMemory($handles, 0x0534940, 'DWORD', $offsets = 0)
						Sleep($DialogSleep)
						If $AmountDialog = 1 Then
						...
						EndIf
						$PlayerInvWHSlot += 1
					ElseIf $InventoryBag[$i][0] = 65535 Then
						$PlayerInvWHSlot += 1
					EndIf
				Next
			Next
			$RowFinished = True
			;=============================WH Deposit Amount============================================
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "WH: Deposit All") > 1 Then
			$LIndex = GUICtrlCreateListViewItem(">>> " & $CurrentScriptRow & " WH: Deposit All", $ScriptConsole)
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
				$PlayerInvWHSlot = 1
				For $i = 1 To UBound($InventoryBag) - 1 Step 1 ; loop through slots of  invent bag
					ScanInventoryBag()
					If $InventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $InventoryBag[$i][0] <> 0 Then
						If Not WinActive($NameOfSomaWindow) Then
							WinActivate($NameOfSomaWindow)
							WinWaitActive($NameOfSomaWindow)
						EndIf
						_KDMemory_WriteProcessMemory($handles, 0x053598C, 'DWORD', $i - $PlayerInvWHSlot, $offsets = 0)
						MouseClick("Left", $WHDepositButtonX, $WHDepositButtonY, 5)
						Sleep($DialogSleep)
						$AmountDialog = _KDMemory_ReadProcessMemory($handles, 0x0534940, 'DWORD', $offsets = 0)
						If $AmountDialog = 1 Then
							Send('{ENTER}')
			...
			$RowFinished = True
			;=============================WH Withdraw Amount============================================
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "WH: Withdraw Amount") > 1 Then
...
;~ 				For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
					If $WHInventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $WHInventoryBag[$i][0] <> 0 Then
						...
						Sleep($DialogSleep)
						If ($WHInventoryBag[$i][2] * $SA[$CurrentScriptRow][$d + 4]) + $BWCurrent <= $BWMax Then ; Weight * Amount = < available BW
							MouseClick("Left", $WHWithdrawButtonX, $WHWithdrawButtonY, 5)
							Sleep($DialogSleep)
							$AmountDialog = _KDMemory_ReadProcessMemory($handles, 0x0534940, 'DWORD', $offsets = 0)
							If $AmountDialog = 1 Then
								If $WHInventoryBag[$i][3] < $SA[$CurrentScriptRow][$d + 4] Then ; if the amount in WH is lower than our amount then take that amount anyway
									Send($WHInventoryBag[$i][3])
									Send('{ENTER}')
								Else
									Send($SA[$CurrentScriptRow][$d + 4])
									Send('{ENTER}')
						...
						ExitLoop
					EndIf
					If $i = UBound($WHInventoryBag) - 1 And $d = 4 And $WHInventoryBag[$i][0] <> $SA[$CurrentScriptRow][$d] Then
						$boton = 0
						MsgBox(0, "", "Could Not Find Item " & $SA[$CurrentScriptRow][$d])
						$RowFinished = True
					EndIf
...
;~ 			MsgBox(0, "", "Max BW " & $MaxBagWeight & @CRLF & "inventfull " & $InventFull & @CRLF & "Nomoremats " & $NoMoreMats, 3)
		...
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "Click NPC") > 1 Then
			...
			If Not WinActive($NameOfSomaWindow) Then
				...
				WinWaitActive($NameOfSomaWindow)
			EndIf
			_KDMemory_WriteProcessMemory($handles, 0x0532850, 'DWORD', 0, $offsets = 0) ; Select the item
			Do
				Do
					FillEntityArray()
...
;~ 						AttackTarget($TargetEntity)
						Sleep(300)
					Until $TotalDistanceFromNPC <= 6 And $TargetEntity > 0
...
;~ 				sleep(1500)
				...
				EndIf
				$Dialog = 0
				$Dialog = _KDMemory_ReadProcessMemory($handles, 0x0532850, 'DWORD', $offsets = 0)
			Until $Dialog > 0
			$RowFinished = True
			;==============================SEND KEY==========================================
...
;~ 	MouseMove($WHDepositButtonX, $WHDepositButtonY, 5)
	_KDMemory_WriteProcessMemory($handles, 0x053598C, 'DWORD', 0, $offsets = 0)
