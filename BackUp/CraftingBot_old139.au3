...
Func AddNPCButton()
	...
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "Click NPC"
	$SA[$SA[0][0]][1] =$TargetMobArray[0][0]
	$SA[$SA[0][0]][2] = $TargetMobArray[0][1]
	GuiUpdate()
	$CurrentScriptRow = 0
	$ScriptFinished = True
	ScriptLoop()
...
Func SelectNPCButton()
	...
	While $SelectTargetMobLoop = 1
		...
		EndIf
		For $i = 0 To 38 Step 1
			If $EA[$i][6] = 1 And $EA[$i][0] <> 27206 And $EA[$i][0] = $TargetMob Then
				...
				ToolTip("")
				$SelectedNPCLabel = $TargetMobArray[0][1]
				ExitLoop
			EndIf
		Next
	WEnd
