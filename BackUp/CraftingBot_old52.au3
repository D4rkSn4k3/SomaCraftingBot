...
Func Navigation()
	If $boton = 1 And $Navigate = True Then
			$x = Abs($NavNodes[1][0] - $MyX)
			$y = Abs($NavNodes[1][1] - $MyY)
			$NavNodeDistance[1][0] = $x + $y ; Saves the distance
			$NavNodeDistance[1][1] = 1 ; Keeps track of the NavNode number
...
Func Gui()
	...
	GUISetOnEvent($StartStop, "botonoff")
	GUISetState(@SW_SHOW)
...
Func AddNavNodesToScript()
	$SA[0][0] += 1 ; Position in ScriptView
...
;~ 	For $i = 1 To $SA[0][0] Step 1 ; Loop through the ScriptArray
	...
	If StringInStr($SA[$CurrentScriptRow][0], "Navigate") > 1 Then
		...
		For $n = 1 To $SA[$CurrentScriptRow][1] Step 1 ; For 1 to Number of NavNodes in SA
			If $SA[$CurrentScriptRow][$n] <> "" Then
				$NavNodes[$n][0] = $SA[$CurrentScriptRow][$n + 1]
				$NavNodes[$n][1] = $SA[$CurrentScriptRow][$n + 1 + $SA[$i][1]]
			EndIf
		Next
		$Navigate = True
	EndIf
