...
;~ 							If ($InventoryBag[$i][2] * $SA[$CurrentScriptRow][$d + 4]) + $BWCurrent <= $BWMax Then
					...
					If $i = UBound($InventoryBag) - 1 And $InventoryBag[$i][0] <> $SA[$CurrentScriptRow][$d] Then
						$boton = 0
						MsgBox(0, "", "Could Not Find Item " & $InventoryBag[$i][0])
						$RowFinished = True
					EndIf
				Next
			Next
...
;~ 				For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
					...
					If $i = UBound($WHInventoryBag) - 1 And $WHInventoryBag[$i][0] <> $SA[$CurrentScriptRow][$d] Then
						$boton = 0
						MsgBox(0, "", "Could Not Find Item " & $WHInventoryBag[$i][0])
						$RowFinished = True
					EndIf
				Next
			Next
