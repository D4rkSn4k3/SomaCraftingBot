...
;~ 		============================================= Go To Nearest Node
		...
		If $TargetNodeDistance <= $PlayertoNodeDistsetting Then
			...
			GUICtrlCreateListViewItem("Moving to X: " & $NavNodes[$Node][0] & " Y: " & $NavNodes[$Node][1], $ScriptConsole)
...
Func Test()
	For $i = 1 To 30 Step 1
		GUICtrlCreateListViewItem($i, $ScriptConsole)
	Next
EndFunc   ;==>Test

Func ScriptLoop()
;~ 	For $i = 1 To $SA[0][0] Step 1 ; Loop through the ScriptArray
;~ msgbox(0,"",$CurrentScriptRow & "   " &  $SA[0][0])
	If $CurrentScriptRow < $SA[0][0] Then
		$CurrentScriptRow += 1
	ElseIf $CurrentScriptRow = $SA[0][0] Then
		$boton = 0
		GUICtrlCreateListViewItem("Script Finished", $ScriptConsole)
		$ScriptFinished = True
	EndIf
	;-------------------------------------------------------------------------
	If $ScriptFinished = False And $SA[0][0] > 0 Then
		If StringInStr($SA[$CurrentScriptRow][0], "Navigate") > 1 Then
			GUICtrlCreateListViewItem("Script Row: " & $CurrentScriptRow & " Navigate", $ScriptConsole)
