...
Func SaveScript()
	...
	Else
		...
		if isarray($IniSectionNames[0]) then
		For $d = 1 To $IniSectionNames[0] Step 1
...
Func Test()
	For $i = 1 To 10 Step 1
;~ 			$LIndex = _GUICtrlListBox_GetTopIndex(GUICtrlGetHandle($ScriptViewList)) ; Autoscroll to end of ListBox !
;~ 			$LIndex = _GUICtrlListBox_SetTopIndex($ScriptViewList,$CurrentScriptRow)
;~ msgbox(0,"",$LIndex,1)
;~ 		$LIndex = GUICtrlCreateListViewItem($i, $ScriptConsole)
;~ 		_GUICtrlListView_EnsureVisible ( $ScriptConsole, $CurrentScriptRow)
		_GUICtrlListView_EnsureVisible($ScriptViewList, $CurrentScriptRow)
		ToolTip($CurrentScriptRow)
	Next
EndFunc   ;==>Test
