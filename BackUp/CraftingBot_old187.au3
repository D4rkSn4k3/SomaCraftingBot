...
Func Gui()
	...
	GUICtrlSetData($ItemsSelectAction, "Craft: Potion Table|WH: Withdraw Amount|WH: Deposit All")
...
;~ 						If $SA[$CurrentScriptRow][$d + 4] > 1 And $WHInventoryBag[$i][3] > 1 Then
							If ($WHInventoryBag[$i][2] * $SA[$CurrentScriptRow][$d + 4]) + $BWCurrent <= $BWMax Then ; check we have enough bw to carry the amount
