...
Func Gui()
	$SendKeyGUI = GUICreate($word,100,120,-1,-1)
$Key = GUICtrlCreateInput("Key",10,5,90,20)
	...
	GUICtrlSetOnEvent($AddKey, "AddSendKeyToScript")
...
Func AddSendKeyToScript()
	...
	Else
		ReDim $SA[UBound($SA) + 1][UBound($SA, 2)] ; else increase rows by one for new data and keep columns the same
	EndIf
	$SA[$SA[0][0]][0] = $SA[0][0] & " | " & "Send Key " $Key
	$SA[$SA[0][0]][1] = $Key
	GuiUpdate()
	$CurrentScriptRow = 0
	$ScriptFinished = True
