	...
	For $i = 1 To 10 Step 1 ; loop thourgh slots of craft window invent bag
		For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
			If $CraftInventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $CraftInventoryBag[$i][0] <> 0 Then
				...
				EndIf
if $i = 1 then
				MouseClickDrag("Left", $PotionCraftTopLeftBoxX , $PotionCraftTopLeftBoxY, $PotionCraftTopLeftBoxX + 55, $PotionCraftTopLeftBoxY - 197, 5) ; drag to active crafting bar
Else
								MouseClickDrag("Left", $PotionCraftTopLeftBoxX+($i* , $PotionCraftTopLeftBoxY, $PotionCraftTopLeftBoxX + 55, $PotionCraftTopLeftBoxY - 197, 5) ; drag to active crafting bar

				endif
;~ 				Sleep(100)
;~ 				Send('{ENTER}')
;~ 				MouseMove($PotionCraftTopLeftBoxX + 217, $PotionCraftTopLeftBoxY - 151, 5)
;~ 				Do
;~ 					FillEntityArray()
;~ 					MouseDown("Left")
