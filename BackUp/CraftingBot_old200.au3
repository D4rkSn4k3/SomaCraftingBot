...
Func Gui()
	...
	GUICtrlSetData($ItemsSelectAction, "Craft: Potion Table|WH: Withdraw Amount|WH: Deposit All|Shop: Sell All")
...
Func ScriptLoop()
	...
	If $ScriptFinished = False And $SA[0][0] > 0 Then
		;=============================Loop============================================
		If StringInStr($SA[$CurrentScriptRow][0], "Loop") > 1 Then
...
;~ 						msgbox(0,"","fired")
		...
		ElseIf StringInStr($SA[$CurrentScriptRow][0], "WH: Deposit All") > 1 Then
			...
			_GUICtrlListView_EnsureVisible($ScriptConsole, $LIndex - 46)
			$PlayerInvWHSlot = 1
;~ 			$InventSpacesFilledPre = _KDMemory_ReadProcessMemory($handles, 0x0535974, 'DWORD', $offsets = 0)
;~ 			$InventSpacesFilled = "string2"
			For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
...
;~ 						If $InventoryBag[$i][3] > 1 Then
						...
						If $AmountDialog = 1 Then
							...
							Sleep($DialogSleep)
						EndIf
;~ 							$InventSpacesFilled = _KDMemory_ReadProcessMemory($handles, 0x0535974, 'DWORD', $offsets = 0)
;~ 							Sleep($DialogSleep)
;~ 							If $InventSpacesFilled = $InventSpacesFilledPre Then
;~ 								$boton = 0
;~ 								MsgBox(0, "", "Not enough Space in WH" & $SA[$CurrentScriptRow][$d])
;~ 								$RowFinished = True
;~ 								ExitLoop 2
;~ 							EndIf
;~ 						Else
...
;~ 						EndIf
					ElseIf $InventoryBag[$i][0] = 65535 Then
						$PlayerInvWHSlot += 1
					EndIf
;~ 					If $i = UBound($InventoryBag) - 1 And $d = 4 Then
;~ 						$RowFinished = True
;~ 					EndIf
				Next
			Next
...
;~ 				For $d = 1 To 4 Step 1 ; loop through SA and compare craft bag invent to our SA data
					If $WHInventoryBag[$i][0] = $SA[$CurrentScriptRow][$d] And $WHInventoryBag[$i][0] <> 0 Then
						...
						Sleep($DialogSleep)
;~ 						If $SA[$CurrentScriptRow][$d + 4] > 1 And $WHInventoryBag[$i][3] > 1 Then
