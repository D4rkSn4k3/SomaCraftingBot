...
Func ScanShopInventory()
	...
	For $i = 1 To UBound($ScanShopInventory) - 1 Step 1
		If $i > 1 Then
				...
				For $i = 1 To UBound($ShopInventory) - 1 Step 1 ; loop thourgh slots of wh invent bag
					If $ShopInventory[$i][0] = $SA[$CurrentScriptRow][$d] And $ShopInventory[$i][0] <> 0 Then
						If Not WinActive($NameOfSomaWindow) Then
							...
							WinWaitActive($NameOfSomaWindow)
						EndIf
						_KDMemory_WriteProcessMemory($handles, 0x053598C, 'DWORD', $i - 1, $offsets = 0) ; Select the item
