...
Func StaffAura()
	If $StaffAuraToggle = 1 Then
		$staffaura = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $staffauraoffset) ; Is there an aura?
		$stance = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $stanceoffset) ; peace/mob/pk
		$eqippedweap = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $eqippedweapoffset) ; What weap? 6 = staff
		Local $EntityCursorTooltip = _KDMemory_ReadProcessMemory($handles, 0x06d85F8, 'DWORD', $offsets = 0) ;read the ID of the mob possibly under the cursor
		If $staffaura = 0 And $stance <> 0 And $eqippedweap = 6 And WinActive($NameOfSomaWindow) Then
			$magicslot = _KDMemory_ReadProcessMemory($handles, 0x05366ac, 'DWORD', $offsets = 0) ; What slot is selected currently
;~ 			$currentslot = $magicslot
			$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $StaffAuraSlot, $offsets = 0)
			WinActivate("Soma Mythological Biography");
			Local $EntityCursorTooltip = _KDMemory_ReadProcessMemory($handles, 0x06d85F8, 'DWORD', $offsets = 0) ;read the ID of the mob possibly under the cursor
;~ 			If $EntityCursorTooltip <> 4294967295 Then
			Local $aPos = MouseGetPos()
			Do
				ToolTip("")
				MouseMove(Random($Reswidth - 150 + $WindowPosition[0], $Reswidth + 150 + $WindowPosition[0], 0), Random($Resheight - 150 + $WindowPosition[1], $Resheight + 150 + $WindowPosition[1], 0), 0)
				Local $EntityCursorTooltip = _KDMemory_ReadProcessMemory($handles, 0x06d85F8, 'DWORD', $offsets = 0) ;read the ID of the mob possibly under the cursor
			Until $EntityCursorTooltip = 4294967295
			Send('{SHIFTDOWN}')
			MouseClick("right")
			Sleep("100")
;~ 			$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $currentslot, $offsets = 0)
			$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $magicslot, $offsets = 0)
			Send('{SHIFTUP}')
...
Func StaffAuraToggle()
	If $StaffAuraToggle = 0 Then
		$StaffAuraToggle = 1
		GUICtrlSetData($StaffAuraLabel, "Staff Aura(ALT + I): " & $StaffAuraToggle)
	ElseIf $StaffAuraToggle = 1 Then
		$StaffAuraToggle = 0
		GUICtrlSetData($StaffAuraLabel, "Staff Aura(ALT + I): " & $StaffAuraToggle)
	EndIf
EndFunc   ;==>StaffAuraToggle



Func IntMode($TargetEntity)
;~ 	$MyUniqueID = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $UniqueID)
	If $WeakenMode = 1 And $EA[$TargetEntity][28] = 0 And $WeakenArray[0] = 0 Then
		$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $WeakenSpellSlot, $offsets = 0)
		MouseClick("Right")
		$WeakenArray[0] = $EA[$TargetEntity][0]
		$WeakenArray[1] = TimerInit()
	ElseIf $WeakenMode = 1 And $WeakenArray[0] > 0 And $EA[$TargetEntity][13] = 0 Then
		$WeakenArray[2] = TimerDiff($WeakenArray[1]) ; check time
		If $WeakenArray[2] > 3250 Then
			$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $WeakenSpellSlot, $offsets = 0)
			MouseClick("Right")
			$WeakenArray[0] = 0
			$WeakenArray[1] = 0
			$WeakenArray[2] = 0
		EndIf
	ElseIf $WeakenMode = 1 And $EA[$TargetEntity][13] = 1 And $EA[$TargetEntity][26] = $MyID Then
;~ 	ElseIf $WeakenMode = 1 And $WeakenArray[0] = 0 And $EA[$TargetEntity][13] = 1 And $EA[$TargetEntity][26] = $MyUniqueID Then
		$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $AttackSpellSlot, $offsets = 0)
		MouseClick("Right")
	ElseIf $WeakenMode = 0 Or $EA[$TargetEntity][28] = 1 Then
		$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $AttackSpellSlot, $offsets = 0)
		MouseClick("Right")
	EndIf
...
Func IntModeToggle()
	If $IntModeToggle = 0 Then
		$IntModeToggle = 1
		GUICtrlSetData($IntModeLabel, "Int Mode(ALT + U): " & $IntModeToggle)
		$BowModeSwitch = 0
		GUICtrlSetData($BowModeSwitchLabel, "Bow Mode(ALT + P): " & $BowModeSwitch)
	ElseIf $IntModeToggle = 1 Then
		$IntModeToggle = 0
		GUICtrlSetData($IntModeLabel, "Int Mode(ALT + U): " & $IntModeToggle)
	EndIf
EndFunc   ;==>IntModeToggle



Func WeakenModeToggle()
	If $WeakenMode = 0 Then
		$WeakenMode = 1
		GUICtrlSetData($WeakenModeLabel, "Weaken Mode(ALT + Y): " & $WeakenMode)
		Global $TempAttackDistance = $AttackDistance
;~ 		msgbox(1,"",$WeakenMode)
		$AttackDistance = 8
		GUICtrlSetData($AttackDistanceSettingBox, $AttackDistance)
	ElseIf $WeakenMode = 1 Then
;~ 		msgbox(1,"","sdfsdf")
		$WeakenMode = 0
		GUICtrlSetData($WeakenModeLabel, "Weaken Mode(ALT + Y): " & $WeakenMode)
...
Func MeleeAura()
	$MyAuraActive = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $AuraActiveOffset)
;~ 	$stance = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $stanceoffset)
	If $MeleeAuraSwitch = 1 And $MyAuraActive = 0 And $EA[0][16] <> 0 And $MPCurrent > 10 Then
;~ 		MouseClick("right", Random(0, 5, 1) * 48 + $Reswidth, Random(0, 5, 1) * 24 + $Resheight, 1, 0)
		$slot = _KDMemory_WriteProcessMemory($handles, 0x05366ac, 'DWORD', $AuraSpellSlot, $offsets = 0)
		MouseClick("right")
	EndIf
EndFunc   ;==>MeleeAura
...
Func Potions()
	$MPCurrent = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $MPoffsets)
	$HPCurrent = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $HPoffsets)
	If $AutoHPPots = 1 And $F1Number > 0 Then
		If $HPCurrent < $HPpotionuse And $HPCurrent <> $HPLast Then ; HP
			$HPLast = $HPCurrent
		...
		EndIf
	EndIf
	If $AutoMPPots = 1 And $F2Number > 0 Then
		If $MPCurrent < $MPpotionuse And $MPCurrent <> $MPLast Then ; MP
			$MPLast = $MPCurrent
			If WinActive($NameOfSomaWindow) Then
				Send("{F2}")
			Else
	...
	If WinActive($NameOfSomaWindow) Then
		For $R = 360 To 0 Step $rotationspeedandsmoothness
			...
			If $EA[$TargetEntity][0] = $EntityCursorTooltip Then
				If $IntModeToggle = 1 Then
					IntMode($TargetEntity)
				Else
					MouseClick("Left")
					If $MeleeAuraSwitch = 1 Then
						MeleeAura()
					EndIf
				EndIf
				$GhostDetect = 0
			...
			ElseIf $R = 10 Then
				$GhostDetect += 1
				If $GhostDetect = 1 Then
					$CurrentPotentialGhost = $EA[$TargetEntity][0]
				EndIf
;~ 			tooltip("Current  " & $CurrentPotentialGhost & " | " & $EA[$TargetEntity][0] & @CRLF & $GhostDetect)
				If $CurrentPotentialGhost <> $EA[$TargetEntity][0] Then
					$GhostDetect = 0
				EndIf
				If $GhostDetect = 10 Then
					For $i = 0 To 6
						If $GhostArray[$i][0] = 0 Then
							$GhostArray[$i][0] = $EA[$TargetEntity][0]
;~ 						msgbox(1,"",$EA[$TargetEntity][0])
							$GhostArray[$i][1] = TimerInit()
							$GhostDetect = 0
							ExitLoop
						EndIf
					Next
...
Func CalcNavWithinDistance($TargetX, $TargetY, $TargetAttackDistance)
	Local $NewCoordsX = $MyX
	Local $NewCoordsY = $MyY
...
;~ 	msgbox(1,"",)
	If $TargetX <= $MyX Then ; if the mob is placed less than chars current X Coords
		Do
			$NewCoordsX -= 1
;~ 			msgbox(1,"",$MyX2)
		Until Abs($TargetX - $NewCoordsX) <= $TargetAttackDistance
		If $TargetY > $MyY Then
			Do
				$NewCoordsY += 1
			Until Abs($TargetY - $NewCoordsY) <= $TargetAttackDistance
		EndIf
		If $TargetY < $MyY Then
			Do
				$NewCoordsY -= 1
	...
	ElseIf $TargetX > $MyX Then
		...
		EndIf
	EndIf
	Global $NearTargetX = $NewCoordsX
	Global $NearTargetY = $NewCoordsY
;~ 	msgbox(1,"",$NearTargetX & " | " & $NearTargetY)
;~ 	tooltip($NearTargetX & " | " & $NearTargetY & @crlf & $EA[$i][2] & " | " & $EA[$i][3],($NearTargetX - $MyX) * 48 + $Reswidth + $WindowPosition[0], ($NearTargetY - $MyY) * 24 + $Resheight + $WindowPosition[1])
;~ 	msgbox(1,"",$NearTargetX & " | " & $NearTargetY)
EndFunc   ;==>CalcNavWithinDistance
...
Func DeadMobArray()
	For $d = 0 To 38 Step 1
		; if a mob reappears after being killed, clear the deadmobarray
		For $z = 0 To 38 Step 1 ; loop through dead mob array
			If $EA[$d][0] = $DeadMobArray[$z][0] And $EA[$d][0] <> 15 Then ;if mob is in deadmobarray and it is not dead
				$DeadMobArray[$z][0] = 0
				$DeadMobArray[$z][1] = 0
				$DeadMobArray[$z][2] = 0
			...
			EndIf
		Next
		; if mob exists in deadmobarray but not in EA
		For $z = 0 To 38 Step 1 ; loop through deadmob array
			If $DeadMobArray[$z][0] > 0 Then ; if a mob exists in deadmobarray
				For $x = 0 To 38 Step 1
					If $DeadMobArray[$z][0] = $EA[$x][0] Then
						ExitLoop 2
					ElseIf $x = 38 And $DeadMobArray[$z][0] <> $EA[$x][0] Then
						$DeadMobArray[$z][0] = 0
						$DeadMobArray[$z][1] = 0
						$DeadMobArray[$z][2] = 0
					EndIf
			...
			EndIf
		Next
		;main dead mob array adding and timer loop
		For $d = 0 To 38 Step 1
			If $EA[$d][5] = 15 Then; if a mob is dead
				For $j = 0 To 38 Step 1 ; loop through Deadmobarray
					If $EA[$d][0] = $DeadMobArray[$j][0] Then ; if dead mob exists in array already
						$DeadMobArray[$j][2] = TimerDiff($DeadMobArray[$j][1]) ;Calculatetime difference
						If $DeadMobArray[$j][2] > 3500 Then
							For $l = 0 To 29 Step 1
								$EA[$d][$l] = 0
;~ 									tooltip("Zeroed")
							Next
						EndIf
						ExitLoop
					ElseIf $j = 38 Then ; if dead mob does not exist in the array
						For $k = 0 To 38 Step 1 ; loop through deadmob array
							If $DeadMobArray[$k][0] = 0 Then ; on the first empty space
								$DeadMobArray[$k][0] = $EA[$d][0]
								$DeadMobArray[$k][1] = TimerInit()
								ExitLoop 2
							EndIf
						Next
					EndIf
		...
		Next
	Next
EndFunc   ;==>DeadMobArray



Func ContestedMob()
	;===== Do not target mobs far away that have taken a hit
;~ if mob has no owner and is outside attackdistance and is taking hits
;~ 	start 5s timer, place flag to ignore
;~ endif

	; ====Check if mob is in ContestedArray and add to EA
	For $i = 0 To 38 Step 1
		If $MobContestedArray[$i][0] > 0 Then
			For $d = 0 To 38 Step 1
				If $EA[$d][0] = $MobContestedArray[$i][0] Then
					$EA[$d][24] = 7
					$EA[$d][29] = 1
				EndIf
			Next
		EndIf
	Next
	;===== Adds the mob to Contested Array
	For $i = 0 To 38 Step 1 ; loop EA
		If ($EA[$i][26] = 0 Or $EA[$i][26] = "") And $EA[$i][1] > $AttackDistance And $EA[$i][5] = 13 And $EA[$i][29] <> 1 And $EA[$i][24] > 0 And $EA[$i][24] < 7 Then ; if mob gets hit out of attack distance
			For $d = 0 To 38 Step 1 ; loop MobContested Array
				If $MobContestedArray[$d][0] = 0 Then ; find an empty space
					$MobContestedArray[$d][0] = $EA[$i][0] ;  add mobs UID to array
					$MobContestedArray[$d][1] = TimerInit() ; Start Timer
					ExitLoop
				EndIf
			Next
		EndIf
	Next
	;===== Checks Timers/Mob Status and Removes Mobs from Contested Array
	For $i = 0 To 38 Step 1 ; loop Contested Array
		If $MobContestedArray[$i][0] > 0 Then
			$MobContestedArray[$i][2] = TimerDiff($MobContestedArray[$i][1])
			If $MobContestedArray[$i][2] > 8000 Then
				$MobContestedArray[$i][0] = 0
				$MobContestedArray[$i][1] = 0
				$MobContestedArray[$i][2] = 0
			EndIf
		Else
			ExitLoop
		EndIf
	Next
EndFunc   ;==>ContestedMob

;~ |0 UID|1Dist|2X |3Y |4 MobID|5 Anim|6 NPCorMob|7 Name|8 HP|9 MHP|10 Stam|11 OX|12 OY|
;~ |13 Speed|14 Animstate2|15 Staff Aura|16 Stance|17 Lvl|18 Grey|19 Invis|20 BW|21 MBW|22 MP|23 MMP
;~ |24 Priority|25 Direction|26 id of owner of mob|27 Owners Name|
Func AntiMobsteal()
	For $t = 0 To 38 Step 1 ; Loop through Entity Array
		If $EA[$t][26] = "" Then
			$EA[$t][26] = 0
		EndIf
		;if player is toe to toe with a mob and both facing eachother then the player has ownership
		If $EA[$t][4] = 0 And $EA[$t][6] = 0 Then ; if it is a player then continue; t = player
			For $m = 0 To 38 Step 1 ; m = mob
				If $EA[$m][24] > 0 And $EA[$m][24] < 7 Then
					If $EA[$m][2] = $EA[$t][2] And $EA[$m][3] = $EA[$t][3] + 2 And $EA[$t][25] = 0 And $EA[$m][25] = 4 And $EA[$m][26] = 0 Then
;~ 							ToolTip("Toe to Toe")
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
						$AntiMobstealArray[$t][4] = "Toe to Toe" ; Type of Ownership
						$EA[$m][26] = $EA[$t][0] ; Save Owners ID
						$EA[$m][27] = $EA[$t][7] ; Save Owners Name

					EndIf
					If $EA[$m][2] = $EA[$t][2] - 1 And $EA[$m][3] = $EA[$t][3] + 1 And $EA[$t][25] = 1 And $EA[$m][25] = 5 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] - 2 And $EA[$m][3] = $EA[$t][3] And $EA[$t][25] = 2 And $EA[$m][25] = 6 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] - 1 And $EA[$m][3] = $EA[$t][3] - 1 And $EA[$t][25] = 3 And $EA[$m][25] = 7 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] And $EA[$m][3] = $EA[$t][3] - 2 And $EA[$t][25] = 4 And $EA[$m][25] = 0 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] + 1 And $EA[$m][3] = $EA[$t][3] - 1 And $EA[$t][25] = 5 And $EA[$m][25] = 1 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] + 2 And $EA[$m][3] = $EA[$t][3] And $EA[$t][25] = 6 And $EA[$m][25] = 2 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
					...
					EndIf
					If $EA[$m][2] = $EA[$t][2] + 1 And $EA[$m][3] = $EA[$t][3] + 1 And $EA[$t][25] = 7 And $EA[$m][25] = 3 And $EA[$m][26] = 0 Then
						$AntiMobstealArray[$t][0] = $EA[$m][0] ; Save mobs ID
						$AntiMobstealArray[$t][1] = $EA[$m][7] ; Save mobs Name
						$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
						$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
			...
			Next
		EndIf
;~ 		If $EA[$t][5] = 15 And $EA[$t][24] = 7 Then
;~ 			ToolTip("something Died")
;~ 		EndIf

		; If = player | if anim = attacking | if nporMob(0 = player) ; detecting if a player is attacking a mob
;~ Player Attacking
		If $EA[$t][6] = 0 And ($EA[$t][5] = 7 Or $EA[$t][5] = 11 Or $EA[$t][5] = 10 Or $EA[$t][5] = 8 Or $EA[$t][5] = 9 Or $EA[$t][5] = 6) Then
;~ 		If $EA[$t][4] = 0 And $EA[$t][6] = 0 And $EA[$t][7] = "SPECTRE" And ($EA[$t][5] = 7 Or $EA[$t][5] = 11 Or $EA[$t][5] = 10 Or $EA[$t][5] = 8 Or $EA[$t][5] = 9 Or $EA[$t][5] = 6) Then
;~ 			msgbox(1,"","Bop",1)
			If $EA[$t][25] = 0 Then ; if facing down
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] + 2 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] + 4 ; Save Y
				$EntityDirectionCoordsArray[2][1] = $EA[$t][2] - 1 ; Save X
				$EntityDirectionCoordsArray[2][2] = $EA[$t][3] + 5 ; Save Y
				$EntityDirectionCoordsArray[3][1] = $EA[$t][2] + 1 ; Save X
				$EntityDirectionCoordsArray[3][2] = $EA[$t][3] + 5 ; Save Y
				$EntityDirectionCoordsArray[4][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[4][2] = $EA[$t][3] + 6 ; Save Y
				For $u = 0 To 4 Step 1 ; loop through coordsarray inside player direction cone
					For $y = 0 To 38 Step 1 ; loop through entity array to scan for Mobs inside direction cone of player
						If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][5] = 15 Then
							ExitLoop 2
						EndIf
						If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][24] > 0 And $EA[$y][24] < 7 And $EA[$y][26] = 0 Then
;~ 							ToolTip("Player Attacking")
							$AntiMobstealArray[$t][0] = $EA[$y][0] ; Save mobs ID
							$AntiMobstealArray[$t][1] = $EA[$y][7] ; Save mobs Name
							$AntiMobstealArray[$t][2] = $EA[$t][0] ; Save Owners ID
							$AntiMobstealArray[$t][3] = $EA[$t][7] ; Save Owners Name
							$AntiMobstealArray[$t][4] = "Player Attacking" ; Type of Ownership
							$EA[$y][26] = $EA[$t][0] ; Save Owners ID
							$EA[$y][27] = $EA[$t][7] ; Save Owners Name
							ExitLoop 2
						ElseIf $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][24] > 0 And $EA[$y][24] < 7 And $EA[$y][26] = $EA[$t][0] Then
							ExitLoop 2
						EndIf
					Next
				Next
			EndIf
			If $EA[$t][25] = 1 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] - 1 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] + 1 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] - 2 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] + 2 ; Save Y
				$EntityDirectionCoordsArray[2][1] = $EA[$t][2] - 1 ; Save X
				$EntityDirectionCoordsArray[2][2] = $EA[$t][3] + 3 ; Save Y
				$EntityDirectionCoordsArray[3][1] = $EA[$t][2] - 3 ; Save X
				$EntityDirectionCoordsArray[3][2] = $EA[$t][3] + 3 ; Save Y
				$EntityDirectionCoordsArray[4][1] = $EA[$t][2] - 2 ; Save X
				$EntityDirectionCoordsArray[4][2] = $EA[$t][3] + 4 ; Save Y
				For $u = 0 To 4 Step 1 ; loop through coords inside player direction cone
					For $y = 0 To 38 Step 1 ; loop through entity array to scan for Mobs inside direction cone of player
						If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][5] = 15 Then
							ExitLoop 2
						EndIf
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 2 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] - 2 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] - 3 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] + 1 ; Save Y
				$EntityDirectionCoordsArray[2][1] = $EA[$t][2] - 3 ; Save X
				$EntityDirectionCoordsArray[2][2] = $EA[$t][3] - 1 ; Save Y
				$EntityDirectionCoordsArray[3][1] = $EA[$t][2] - 4 ; Save X
				$EntityDirectionCoordsArray[3][2] = $EA[$t][3] ; Save Y
				$EntityDirectionCoordsArray[4][1] = $EA[$t][2] - 4 ; Save X
				$EntityDirectionCoordsArray[4][2] = $EA[$t][3] + 2 ; Save Y
				$EntityDirectionCoordsArray[5][1] = $EA[$t][2] - 4 ; Save X
				$EntityDirectionCoordsArray[5][2] = $EA[$t][3] - 2 ; Save Y
				$EntityDirectionCoordsArray[6][1] = $EA[$t][2] - 5 ; Save X
				$EntityDirectionCoordsArray[6][2] = $EA[$t][3] + 1 ; Save Y
				$EntityDirectionCoordsArray[7][1] = $EA[$t][2] - 5 ; Save X
				$EntityDirectionCoordsArray[7][2] = $EA[$t][3] - 1 ; Save Y
				$EntityDirectionCoordsArray[8][1] = $EA[$t][2] - 6 ; Save X
				$EntityDirectionCoordsArray[8][2] = $EA[$t][3] ; Save Y
				For $u = 0 To 8 Step 1 ; loop through coords inside player direction cone
					For $y = 0 To 38 Step 1 ; loop through entity array to scan for Mobs inside direction cone of player
						If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][5] = 15 Then
							ExitLoop 2
						EndIf
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 3 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] - 1 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] - 1 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] - 2 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] - 2 ; Save Y
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 4 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] - 2 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] - 4 ; Save Y
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 5 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] + 1 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] - 1 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] + 2 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] - 2 ; Save Y
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 6 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] + 2 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] + 3 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] - 1 ; Save Y
...
;~ 							ToolTip("Player Attacking")
				...
				Next
			EndIf
			If $EA[$t][25] = 7 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] + 1 ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] + 1 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] + 2 ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] + 2 ; Save Y
...
;~ 		For $o = 1 To 6 Step 1 ; loop through targetmob array
;~ 		; if the mob has attacked and it is one of the target mobs and it is not a player and it is a mob
;~ 			If $EA[$t][4] = $TargetMobArray[$o][0] And $EA[$t][7] = $TargetMobArray[$o][1] And $EA[$t][4] <> 0 And $EA[$t][6] = 1 And $EA[$t][26] = 0 Then
;~ 				For $h = 0 To 38 Step 1
;~ 					If $AntiMobstealArray[$h][0] = $EA[$t][0] Then
;~ 						$MobHasOwner = True
;~ 						ExitLoop
;~ 					ElseIf $h = 38 And $AntiMobstealArray[$h][0] <> $EA[$t][0] Then
;~ 						$MobHasOwner = False
;~ 					EndIf
;~ 				Next
;~ 				If $AntiMobstealArray[$t][4] <> $EA[$t][25] Then
;~ 					$MobHasOwner = False
;~ 				EndIf
;~ 				If $EA[$t][5] = 7 And $MobHasOwner = False Then ; If mob animation is "Attacking"
;~ 				If $EA[$t][5] = 7 Then ; If mob animation is "Attacking"

		; if entity is attacking | if = not player | if = has no owner | if = a targetmob
		If $EA[$t][5] = 7 And $EA[$t][24] > 0 And $EA[$t][24] < 7 Then
;~ 		If $EA[$t][5] = 7 And $EA[$t][24] > 0 And $EA[$t][24] < 7 And $EA[$t][7] = "LICH" Then

			If $EA[$t][25] = 0 Then ; if facing down then prepare coords for scanning into array
				$EntityDirectionCoordsArray[0][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[0][2] = $EA[$t][3] + 2 ; Save Y
				$EntityDirectionCoordsArray[1][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[1][2] = $EA[$t][3] + 4 ; Save Y
				...
				$EntityDirectionCoordsArray[4][1] = $EA[$t][2] ; Save X
				$EntityDirectionCoordsArray[4][2] = $EA[$t][3] + 6 ; Save Y
				;Mob attacks, if it has no owner, set the owner
				If $EA[$t][26] = 0 Then
					For $u = 0 To 4 Step 1 ; loop through coords inside mob direction cone
						For $y = 0 To 38 Step 1 ; loop through entity array to scan for players inside direction cone of mob
							If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][4] = 0 Then
;~ 							ToolTip("Mob Attacking")
								$AntiMobstealArray[$t][0] = $EA[$t][0] ; Save mobs ID
								$AntiMobstealArray[$t][1] = $EA[$t][7] ; Save mobs Name
								$AntiMobstealArray[$t][2] = $EA[$y][0] ; Save Owners ID
								$AntiMobstealArray[$t][3] = $EA[$y][7] ; Save Owners Name
								$AntiMobstealArray[$t][4] = "Mob Attacking" ; Type of Ownership
								$AntiMobstealArray[$t][5] = $EA[$t][25] ; Direction
								$EA[$t][26] = $EA[$y][0] ; Save Owners ID
								$EA[$t][27] = $EA[$y][7] ; Save Owners Name
								ExitLoop 2
							EndIf
						Next
					Next
					;Mob attacks, if it has an owner and detects the owner still therethen exitloop(so nothing changes)
				ElseIf $EA[$t][26] <> 0 Then
					For $u = 0 To 4 Step 1 ; loop through coords inside mob direction cone
						For $y = 0 To 38 Step 1 ; loop through entity array to scan for players inside direction cone of mob
							If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][4] = 0 And $EA[$t][26] = $EA[$y][0] Then
;~ 								ToolTip("Owner still here")
								ExitLoop 2
							ElseIf $u = 4 And $y = 38 Then
								For $u = 0 To 4 Step 1 ; loop through coords inside mob direction cone
									For $y = 0 To 38 Step 1 ; loop through entity array to scan for players inside direction cone of mob
										If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][4] = 0 Then
;~ 							ToolTip("Mob Attacking")
...
;~ 							ToolTip("Mob Attacking")
			...
			If $EA[$t][25] = 2 Then ; if facing down then prepare coords for scanning into array
				...
				;Mob attacks, if it has no owner, set the owner
				If $EA[$t][26] = 0 Then
					For $u = 0 To 8 Step 1 ; loop through coords inside mob direction cone
						For $y = 0 To 38 Step 1 ; loop through entity array to scan for players inside direction cone of mob
							If $EntityDirectionCoordsArray[$u][1] = $EA[$y][2] And $EntityDirectionCoordsArray[$u][2] = $EA[$y][3] And $EA[$y][4] = 0 Then
;~ 							ToolTip("Mob Attacking")
								$AntiMobstealArray[$t][0] = $EA[$t][0] ; Save mobs ID
...
;~ 			; If mobs are moving or showing as dead then update antimobstealarray
		If $EA[$t][5] = 4 And $EA[$t][6] = 1 Then
;~ 		If $EA[$t][5] = 4 And $EA[$t][6] = 1 Or ($EA[$t][5] = 15 And $EA[$t][6] = 1) Then
			For $m = 0 To 38 Step 1
				If $AntiMobstealArray[$m][0] = $EA[$t][0] And $AntiMobstealArray[$m][0] <> 0 And $AntiMobstealArray[$m][1] = $EA[$t][7] Then
					$AntiMobstealArray[$m][0] = "" ; Save mobs ID
					$AntiMobstealArray[$m][1] = "" ; Save mobs Name
					$AntiMobstealArray[$m][2] = "" ; Save Owners ID
					$AntiMobstealArray[$m][3] = "" ; Save Owners Name
					$AntiMobstealArray[$m][4] = "" ; Save mobs direction
					$AntiMobstealArray[$m][5] = "" ; Save mobs direction
				EndIf
			Next
		EndIf
;~ 			;Copy data from antimobstealarray to main entityarray
		For $m = 0 To 38 Step 1
			If $AntiMobstealArray[$m][0] = $EA[$t][0] And $AntiMobstealArray[$m][0] <> 0 And $AntiMobstealArray[$m][1] = $EA[$t][7] Then
				$EA[$t][26] = $AntiMobstealArray[$m][2] ; Save Owners ID
				$EA[$t][27] = $AntiMobstealArray[$m][3] ; Save Owners Name
;~ 				;ExitLoop
			EndIf
		Next
	Next
EndFunc   ;==>AntiMobsteal



;=========== Scan for mobs that you have killed to pickup the drops
Func PickupDrops()
	For $i = 0 To 38 Step 1
		; if the mob in the mboarray is dead
		If $EA[$i][0] = $MobDropArray[10][0] And $EA[$i][7] = $MobDropArray[10][1] And $NavigatingToDrop = False And $EA[$i][0] <> 0 And $EA[$i][5] = 15 Then
;~ 			tooltip($MobDropArray[10][0])
			;Clear the drops array
			For $d = 0 To 10 Step 1
				$MobDropArray[$d][0] = 0
				$MobDropArray[$d][1] = 0
			Next
			Sleep(100)
			Send('{SHIFTDOWN}')
			MouseMove(($EA[$i][2] - $MyX) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY) * 24 + $UnalteredResHeight, 0) ;Centre
			Sleep($DropSleep)
			$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])

			If $CursorStatus = 2 Then
				$MobDropArray[1][0] = $EA[$i][2]
				$MobDropArray[1][1] = $EA[$i][3]
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY + 2) * 24 + $UnalteredResHeight, 0) ; down
			Sleep($DropSleep)
			$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])

			If $CursorStatus = 2 Then
				$MobDropArray[2][0] = $EA[$i][2]
				$MobDropArray[2][1] = $EA[$i][3] + 2
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX - 1) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY + 1) * 24 + $UnalteredResHeight, 0) ; Down, Left
			Sleep($DropSleep)
			$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])

			If $CursorStatus = 2 Then
				$MobDropArray[3][0] = $EA[$i][2] - 1
				$MobDropArray[3][1] = $EA[$i][3] + 1
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX - 2) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
			$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])

			If $CursorStatus = 2 Then
				$MobDropArray[4][0] = $EA[$i][2] - 2
				$MobDropArray[4][1] = $EA[$i][3]
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX - 1) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY - 1) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
			$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])

			If $CursorStatus = 2 Then
				$MobDropArray[5][0] = $EA[$i][2] - 1
				$MobDropArray[5][1] = $EA[$i][3] - 1
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY - 2) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
...
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
			...
			If $CursorStatus = 2 Then
				$MobDropArray[6][0] = $EA[$i][2]
				$MobDropArray[6][1] = $EA[$i][3] - 2
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX + 1) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY - 1) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
...
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
			...
			If $CursorStatus = 2 Then
				$MobDropArray[7][0] = $EA[$i][2] + 1
				$MobDropArray[7][1] = $EA[$i][3] - 1
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX + 2) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
...
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
			...
			If $CursorStatus = 2 Then
				$MobDropArray[8][0] = $EA[$i][2] + 2
				$MobDropArray[8][1] = $EA[$i][3]
				$MobDropArray[0][0] += 1
			EndIf
			MouseMove(($EA[$i][2] - $MyX + 1) * 48 + $UnalteredResWidth, ($EA[$i][3] - $MyY + 1) * 24 + $UnalteredResHeight, 0)
			Sleep($DropSleep)
...
;~ 				ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
			...
			If $CursorStatus = 2 Then
				$MobDropArray[9][0] = $EA[$i][2] + 1
				$MobDropArray[9][1] = $EA[$i][3] + 1
				$MobDropArray[0][0] += 1
			EndIf
			Send('{SHIFTUP}')
		EndIf
		;==================== Calculate distance to the drop, if toof ar then navigate to the drop otherwise pick it up
		If $MobDropArray[0][0] > 0 Then
			$NavigatingToDrop = True
			For $f = 1 To 9 Step 1
				If $MobDropArray[$f][0] > 0 And $MobDropArray[$f][1] > 0 Then
					$MyX = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $CoordsXOffsets)
					$MyY = _KDMemory_ReadProcessMemory($handles, 0x06d8600, 'DWORD', $CoordsYOffsets)
					$x = Abs($MobDropArray[$f][0] - $MyX)
					$y = Abs($MobDropArray[$f][1] - $MyY)
					$DistanceFromDrop = $x + $y
					If $DistanceFromDrop > 4 Then
						NavigateToTarget($MobDropArray[$f][0], $MobDropArray[$f][1])
;~ 						ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
						Sleep(800)
						ExitLoop 2
					ElseIf $DistanceFromDrop <= 4 Then
						Send('{SHIFTDOWN}')
						MouseMove(($MobDropArray[$f][0] - $MyX) * 48 + $UnalteredResWidth, ($MobDropArray[$f][1] - $MyY) * 24 + $UnalteredResHeight, 0)
						Sleep($DropSleep2)
						$CursorStatus = _KDMemory_ReadProcessMemory($handles, 0x06dd468, 'DWORD', $CursorStatusoffsets)
;~ 							ToolTip("MobID:  " & $EA[$i][0] & @CRLF & "MyID:  " & $MyID & @CRLF & "Mob Owner:  " & $EA[$i][26] & @CRLF & "Mob Drop:  " & $MobDropArray[10][0] & @CRLF & "Anim:  " & $EA[$i][5], ($EA[$i][2] - $MyX) * 48 + 100 + $Reswidth + $WindowPosition[0], ($EA[$i][3] - $MyY) * 24 + $Resheight + $WindowPosition[1])
						If $CursorStatus = 2 Then ; if no insert fast spiral
							MouseClick("Left")
						EndIf
						$MobDropArray[$f][0] = 0
						$MobDropArray[$f][1] = 0
						...
						If $f = 9 Then
							For $d = 0 To 9 Step 1
								...
								$MobDropArray[$d][1] = 0
							Next
							$NavigatingToDrop = False
							Sleep(500)
;~ 							ToolTip("1" & "   " & $NavigatingToDrop)
;~ 							FillEntityArray()
						EndIf
					EndIf
				ElseIf $f = 9 Then
					...
					$NavigatingToDrop = False
					Sleep(500)
;~ 					ToolTip("2" & "   " & $NavigatingToDrop)
					FillEntityArray()
;~ 					ToolTip("Not navigating anymore")
;~ 							Next
;~ 								$DropScanAndPickup = False
;~ 								$MobDropArray[10][0] = 0
;~ 								FillEntityArray()
;~ 								ExitLoop 2
				EndIf
...
;~ 				$MobDropArray[10][0] = 0
			FillEntityArray()
;~ 				ExitLoop
;~ 		ElseIf $i = 38 And $EA[$i][0] <> $MobDropArray[10][0] Then
;~ 			For $d = 0 To 10 Step 1
;~ 				$MobDropArray[$d][0] = 0
;~ 				$MobDropArray[$d][1] = 0
;~ 			Next
...
Func ArrayDisplay()
	If $ArrayDisplayWindows = False And $DevMode = True Then
		$ArrayDisplayWindows = True
		GUISetState(@SW_SHOW, $AntiMobstealGUI)
		GUISetState(@SW_SHOW, $EntityArrayGUI)
		GUISetState(@SW_ENABLE, $AntiMobstealGUI)
		GUISetState(@SW_ENABLE, $EntityArrayGUI)
		WinActivate($NameOfSomaWindow)
	ElseIf $ArrayDisplayWindows = True And $DevMode = True Then
		$ArrayDisplayWindows = False
		GUISetState(@SW_DISABLE, $AntiMobstealGUI)
		GUISetState(@SW_DISABLE, $EntityArrayGUI)
		GUISetState(@SW_HIDE, $AntiMobstealGUI)
		GUISetState(@SW_HIDE, $EntityArrayGUI)
...
Func AntiMobstealGUI()
	;Save mobs ID | 1 Save mobs Name | 2 Save Owners ID | 3 Save Owners Name | Save mob's direction
;~ 	Global $AntiMobstealGUI = GUICreate("AntiMosteal", 470, 800, 1445, 0, -1, $WS_EX_COMPOSITED)
;~ 	Global $AntiMobstealGUI = GUICreate("AntiMosteal", 470, 850, 0, 0, -1, $WS_EX_COMPOSITED)
	Global $AntiMobstealGUI = GUICreate("AntiMosteal", 470, 850, 1285, 10, -1, $WS_EX_COMPOSITED)
;~ 	Global $AntiMobstealListView = GUICtrlCreateListView("MobID|MobName|OwnerID|OwnerName|MobDirection", 10, 10, 450, 710) ;,$LVS_SORTDESCENDING)
	Global $AntiMobstealListView = GUICtrlCreateListView("MobID|MobName|OwnerID|OwnerName|TypeofOwnership|MobDir", 10, 50, 5000, 770, BitOR($GUI_SS_DEFAULT_LISTVIEW, $LVS_NOCOLUMNHEADER))

EndFunc   ;==>AntiMobstealGUI

;~ |0 UID|1Dist|2X |3Y |4 MobID|5 Anim|6 NPCorMob|7 Name|8 HP|9 MHP|10 Stam|11 OX|12 OY|13 Speed|14 Animstate2|15 Staff Aura|16 Stance|17 Lvl|18 Grey|19 Invis|20 BW|21 MBW|22 MP|23 MMP|24 Priority|25 Direction|26 id of owner of mob|27 Owners Name|

Func EntityArrayGUI()

	;Save mobs ID | 1 Save mobs Name | 2 Save Owners ID | 3 Save Owners Name | Save mob's direction
;~ 	Global $EntityArrayGUI = GUICreate("EntityArray", 1520, 830, 0, 0, -1, $WS_EX_COMPOSITED)
	Global $EntityArrayGUI = GUICreate("EntityArray", 1520, 830, 1922, 79, -1, $WS_EX_COMPOSITED)
	Global $EntityArrayListView = GUICtrlCreateListView("UID|Dist|X|Y|Type|Anim|NPC|NameNameNa|HPh|MHP|Stam|HexHexHex|weap|Speed|Anim2|StafA|Stance|Lvl|Grey|Invis|BW|MBW|MP|MMP|Prior|Dire|OID|OName|Weak", 10, 25, 1500, 800, BitOR($GUI_SS_DEFAULT_LISTVIEW, $LVS_NOCOLUMNHEADER))
;~ 		Global $EntityArraylabel = GUICtrlCreateLabel("  UID        |Dist         |X        |Y    |Type        |Anim          |NPCorM        |Name                 |HP|MHP|Stam|Hex|N/A|Speed|Anim2|Staff A|Stance|Lvl|Grey|Invis|BW|MBW|MP|MMP|Priority|Direc|OwnerID|OwnerName", 10, 10, 1780, 880)
EndFunc   ;==>EntityArrayGUI
;~ Func EntityArrayGUI()

;~ 	;Save mobs ID | 1 Save mobs Name | 2 Save Owners ID | 3 Save Owners Name | Save mob's direction
;~ 	Global $EntityArrayGUI = GUICreate("EntityArray", 1520, 830, 0, 0,-1,$WS_EX_COMPOSITED)
;~ 	Global $EntityArrayListView = GUICtrlCreateListView("UID|Dis|X|Y|Typ|Ani|NPC|Nameeeeee|HP|MHP|Stam|HexHexH|N/A|Speed|Anim2|Staff A|Stance|Lvl|Grey|Invis|BW|MBW|MP|MMP|Priority|Direc|OwnerID|OwnerName", 10, 25, 1500, 800,BitOR($GUI_SS_DEFAULT_LISTVIEW, $LVS_NOCOLUMNHEADER))
;~ 		Global $EntityArraylabel = GUICtrlCreateLabel("  UID        Dist          X         Y     Type        Anim     NPCorM        Name                 HP       MHP        Stam                Hex                          Speed          Anim2        Staff A           Stance           Lvl      Grey         Invis            BW          MBW         MP        MMP       Priority         Direc          OwnerID          OwnerName", 10, 10, 1780, 800)
;~ 			GUISetState(@SW_Show)
;~ EndFunc   ;==>EntityArrayGUI
;~ ,$LVS_EX_DOUBLEBUFFER
